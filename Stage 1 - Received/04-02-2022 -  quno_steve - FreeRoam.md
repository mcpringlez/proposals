...
#### Exception Threshold
If this problem arises and we've exausted all strategies (pausing, staggering, etc) during the purchase of a specific asset and it still doesn't resolve the price discrepancy, this proposal authorizes Boss DAO treasury to purchase the assets on a different marketplace once assets meet or exceed a threshold of a 20% discount from Freeroam floor. This threshold can be amended via future proposals if we find that it proves to be too large or small. Of course this decision and timing will need to be kept private within the small group who are actively purchasing so opportunistic insiders cannot time these decisions.
